...
import org.reasm.commons.parseerrors.LabelExpectedParseError;
...
public class LogicalLineParserTest {
    ...
    @Nonnull
    private static final SubstringBounds[] NO_LABELS = EMPTY_SUBSTRING_BOUNDS_ARRAY;
    @Nonnull
    private static final SubstringBounds[] NO_OPERANDS = EMPTY_SUBSTRING_BOUNDS_ARRAY;
    @Nonnull
    private static final int[] NO_CONTINUATION_CHARACTERS = EMPTY_INT_ARRAY;
    ...
    @Test
    public void parse() {
        final LogicalLineAttributes attributes = LogicalLineParser.parse(new GenericCharSequenceReader(this.text, this.start),
                new Syntax(EMPTY_INT_ARRAY, EMPTY_INT_ARRAY));

        // Check SourceNode properties
        assertThat(attributes.length, is(this.length));
        if (this.parseError == null) {
            assertThat(attributes.parseError, is(nullValue()));
        } else {
            assertThat(attributes.parseError, is(new EquivalentParseError(this.parseError)));
        }

        // Check labels
        assertThat(attributes.labels, is(this.labels));

        // Check mnemonic
        assertThat(attributes.mnemonic, is(this.mnemonic));

        // Check operands
        assertThat(attributes.operands, is(this.operands));

        // Check comment
        assertThat(attributes.comment, is(this.comment));

        // Check continuation characters
        assertThat(attributes.continuationCharacters, is(this.continuationCharacters));

        final LogicalLine logicalLine = TestLogicalLineFactory.INSTANCE.createLogicalLine(attributes);
